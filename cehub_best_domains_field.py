...
class CeHubConnector:
    ...
    def get_cehub_data(self, lat, lon, start_date, end_date, queries):
        """
        ...
        """
        PARAMS = self.build_json_payload(lat, lon, start_date, end_date, queries)
        print(
            f'Getting data for geo location at latitude <{lat}> and longitude <{lon}> for date range from <{start_date}> to <{end_date}>')
        ...
        try:
            ...
            print(
                f'No coordinates was found for geo location at latitude <{lat}> and longitude <{lon}>, exception is {exe}')

    def convert_weather_json_to_dict(self, json_response: List[Any], id_col: str, id_value: str) -> dict:
        """
        Converts weather data REST call JSON response to dictionary.
        ...
        for i in range(len(json_response)):
            ...
            responses[self.lat_col] = coordinates[0]
            responses[self.lon_col] = coordinates[1]

            # dates
            responses[DATES] = json[TIME_INTERVALS][0]  # str(json[TIME_INTERVALS][0]).replace('T0000','')
            # codes
            codes = json[CODES]
            ...
            for j in range(len(codes)):
                ...
                responses[str(codes[j][VARIABLE]).replace(' ', '_') + '_(' + ''.join([agg[0].upper(), agg[1:]]) +
                          ')_(' + unit + ')'] = codes[j][DATA_PER_TIME_INTERVAL][0][DATA][0]

        return responses

    def convert_soil_json_to_dict(self, json_response: List[Any], id_col: str, id_value: str) -> dict:
        """
        Converts soil data REST call JSON response to a dictionary.
        :param json_response: The REST response in JSON.
        :param id_col: Any unique ID from the field file
        :param id_value: The value of the unique ID
        ...
        responses = {id_col: id_value}
        for i in range(len(json_response)):
            json_local = json_response[i]

            # geometry
            geometry = json_local[GEOMETRY]
            coordinates = geometry[COORDINATES][0]
            responses[self.lat_col] = coordinates[0]
            responses[self.lon_col] = coordinates[1]

            # codes
            codes = json_local[CODES]
            for j in range(len(codes)):
                column_name: str
                unit: str = codes[j][UNIT]
                ...
                responses[column_name] = codes[j][DATA_PER_TIME_INTERVAL][0][DATA][0]

        return responses

    @staticmethod
    def load_field_data(input_file_dir: str, source_data_filename: str, sheet_name: str, add_on_cols: dict) -> pd:
        ...
        if pathlib.Path(file_name_path).suffix == '.csv':
            field_df = pd.read_csv(file_name_path)
        else:
            field_df = pd.read_excel(file_name_path, sheet_name=sheet_name)

        counter = 0
        for t in (add_on_cols.items()):
            field_df.insert(counter, t[0], t[1])
            counter += 1

        return field_df

    def field_time_data(self, field_df: pd, interested_dates_cols: list, start_date_offset, end_date_offset) -> pd:
        """
        Forming time data dataframe subset.
        :param field_df: The dataframe of the input CSV file with geolocation information
        ...
        self.convert_to_datetime(field_df, interested_dates_cols)
        ...
        field_df[START_DATE_COLUMN] = field_df.apply(
            lambda x: min(x[interested_dates_cols]) + timedelta(days=start_date_offset), axis=1)
        field_df[END_DATE_COLUMN] = field_df.apply(
            lambda x: max(x[interested_dates_cols]) + timedelta(days=end_date_offset), axis=1)

        field_df[START_DATE_COLUMN] = pd.to_datetime(field_df[START_DATE_COLUMN]).dt.date
        field_df[END_DATE_COLUMN] = pd.to_datetime(field_df[END_DATE_COLUMN]).dt.date

        # Removes 'Unnamed' columns from the dataframe
        field_df.drop(field_df.columns[field_df.columns.str.contains('Unnamed')], axis=1, inplace=True)

        pd.set_option('display.max_rows', 100)
        pd.set_option('display.max_columns', None)

        field_df.info()
        field_df.loc[:, joined_on_cols + start_end_cols]

        field_with_time = field_df[joined_on_cols + start_end_cols]
        field_with_time.drop_duplicates(inplace=True, ignore_index=True)
        field_with_time.info()
...
if __name__ == "__main__":
    ...
    field_data_file_name = os.path.splitext(source_filename)[0]
    field_data_file_name_path = f'{output_dir}{os.path.sep}{field_data_file_name}'
    ...
    id_column = PropertyUtil.get_property(INI_FILE, CEHUB_SECTION, ID_COL)
    ...
    df = CeHubConnector.load_field_data(input_dir, source_filename, sheet_name, internal_cols)
    cehub: CeHubConnector = CeHubConnector(api_key, id_column, lat_column, long_column,
                                           country_code_column)
    print(f'\n=========== Loading {source_filename} {sheet_name} into dataframe ==========')
    field_time: pd = cehub.field_time_data(df, user_interested_date_cols, start_date_offset, end_date_offset)
    total_field: int = len(field_time)

    # Getting weather data from CE Hub
    print(f'\n=========== Getting Weather Data from CE Hub ==========')
    weather_df = pd.DataFrame()
    failed_weather_extractions: int = 0

    for i in range(total_field):
        weather_queries = cehub.build_weather_data_query_best_dataset(field_time[cehub.country_code_col][i],
                                                                      precipitation_dom,
                                                                      temperature_dom, wind_dom)
        weather_response = cehub.get_cehub_data(field_time[cehub.lat_col][i], field_time[cehub.lon_col][i],
                                                field_time[START_DATE_COLUMN][i], field_time[END_DATE_COLUMN][i],
                                                weather_queries)
        try:
            response_dict = cehub.convert_weather_json_to_dict(weather_response, id_column, field_time[cehub.id_col][i])
            each_field_df = pd.DataFrame(response_dict)
            weather_df = weather_df.append(each_field_df, ignore_index=True)
        except Exception as exe:
            print(f"Failed to extract weather data for latitude <{field_time[cehub.lat_col][i]}> "
                  f"and longitude <{field_time[cehub.lon_col][i]}> with error: <{exe}>")
            failed_weather_extractions += 1

    print(
        f'<{failed_weather_extractions}> weather data extractions have failed to retrieve from CE Hub out of <{total_field}>')
    weather_df.info()
    ...
    failed_soil_extractions: int = 0

    for i in range(len(field_time)):
        soil_queries = [cehub.build_soil_query(START_DEPTH_0, END_DEPTH_30),
                        cehub.build_soil_query(START_DEPTH_0, END_DEPTH_60)]
        soil_response = cehub.get_cehub_data(field_time[cehub.lat_col][i], field_time[cehub.lon_col][i],
                                             field_time[START_DATE_COLUMN][i], field_time[END_DATE_COLUMN][i],
                                             soil_queries)
        try:
            response_dict = cehub.convert_soil_json_to_dict(soil_response, id_column, field_time[cehub.id_col][i])
            each_field_df = pd.DataFrame(response_dict)
            soil_df = soil_df.append(each_field_df, ignore_index=True)
        except Exception as exe:
            print(f"Failed to extract soil data for latitude <{field_time[cehub.lat_col][i]}> "
                  f"and longitude <{field_time[cehub.lon_col][i]}> with error: <{exe}>")
            failed_soil_extractions += 1

    print(
        f'<{failed_soil_extractions}> soil data extractions have failed to retrieve from CE Hub out of <{total_field}>')
    soil_df.info()

    print(f'\n========== Writing Weather and Soil Data to {field_data_file_name_path} ==========')
    weather_df.to_csv(field_data_file_name_path + '_weather_data_only_best_domains.csv', index=False,
                      header=weather_df.columns, encoding='UTF-8-sig')
    soil_df.to_csv(field_data_file_name_path + '_soil_data_only.csv', index=False, header=soil_df.columns,
                   encoding='UTF-8-sig')
    print(
        f"Finished writing {field_data_file_name_path + '_weather_data_only_best_domains.csv'} and "
        f"{field_data_file_name_path + '_soil_data_only.csv'}")

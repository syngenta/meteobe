"""Module to retrieve CE Hub weather and soil data by user selected domains"""
import pathlib
...
USER_INTERESTED_DOMAINS = 'user_interested_domains'
...
class PropertyUtil:
    """Gets user input variable values from the cehub_field.ini file"""
...
class CeHubConnector:
    ...
    @staticmethod
    def build_weather_data_query_selected_domain(selected_domain: str) -> list:
        """
        Builds weather data query for CE Hub by using user domain input from ini file.
        :param selected_domain: User selected domain name.
    ...
    def load_trial_data(self, input_file_dir: str, source_data_filename: str, sheet_name: str,
        ...
        # Calculate offset dates from the dates_of_interest columns, and add them back to the dataframe
...
if __name__ == "__main__":
    ...
    sn = PropertyUtil.get_property(INI_FILE, FILE_PATHS_SECTION, SHEET_NAME)
    ...
    # Load interested domains
    user_interested_dom: list = PropertyUtil.get_property(INI_FILE, CEHUB_SECTION, USER_INTERESTED_DOMAINS).split(',')

    cehub: CeHubConnector = CeHubConnector(api_key, trial_column, lat_column, long_column, country_code_column)
    print(f'\n=========== Loading {source_filename} {sn} into dataframe ==========')
    trial_time: pd = cehub.load_trial_data(input_dir, source_filename, sn, user_interested_date_cols,
    ...
    for domain in range(len(user_interested_dom)):
        for i in range(total_trial):
            weather_queries = cehub.build_weather_data_query_selected_domain(user_interested_dom[domain])
        ...
        print(f'<{failed_weather_trials}> trials has failed to retrive from CE Hub out of <{total_trial}>')
        weather_df.info()
        if weather_df.empty:
            print(f'\nNo weather data is found from domain {user_interested_dom[domain]}')
            continue
        ...
        print(f'\n========== Writing Weather Data to {trial_data_file_name_path} ==========')
        weather_df.to_csv(trial_data_file_name_path + '_weather_data_only_' + user_interested_dom[domain] +
                          '.csv', index=False, header=weather_df.columns, encoding='UTF-8-sig')
    ...
    print(f'\n========== Writing Soil Data to {trial_data_file_name_path} ==========')
    soil_df.to_csv(trial_data_file_name_path + '_soil_data_only.csv', index=False, header=soil_df.columns,
                   encoding='UTF-8-sig')
    print(
        f"Finished writing {trial_data_file_name_path + '_weather_data_only.csv'} and "
